def redis_memoize(
    ...
    def decorator(function: Callable):
        def wrapper(*args, **kwargs):
            # Get all args, including default arguments, for checking against conditions
            all_args = extract_all_args(function, args, kwargs, from_decorator=True)
            ...
            pickled_value = redis_backend.r.get(key)
            if pickled_value is not None:
                log.debug(f"Getting memoized data for {key}")
                return pickle.loads(pickled_value)
            else:
                value = function(*args, **kwargs)
                try:
                    log.debug(f"Saving memoized data for {key}")
                    redis_backend.r.set(key, pickle.dumps(value), ex=expiration)
...
def extract_all_args(
    ...
    reverse_defaults.extend([None] * (len(reverse_arg_names) - len(reverse_defaults)))

    # Combine keys and values, undo the reversion and transform back into a dict.
    args_with_values = dict(list(zip(reverse_arg_names, reverse_defaults))[::-1])

    # Arguments provided in `args` are always left-to-right in the overall args list, so we can apply them as such.
    for i, arg in enumerate(args):
        args_with_values[all_arg_names[i]] = arg

    # Arguments in `kwargs` however, are not, so we need to match by argument name.
...
def redis_invalidate_memoize(function_name: str, *args, **kwargs):
    """
    ...
    key_pattern: str = "memoize_" + function_name + "_" + "_".join(hashed_args) + "*"
    keys = redis_backend.r.keys(pattern=key_pattern)
    for key in keys:
        log.debug(f"Deleting memoized key {key}")
        redis_backend.r.delete(key)
    return len(keys)